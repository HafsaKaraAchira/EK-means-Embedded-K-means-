...
void log_error(int point_num, int cluster_num, int it_num,double error){
  FILE *output = fopen ("./results/error.csv", "at" );
  fprintf (output, "%d,%d,%d,%f\n",point_num,cluster_num,it_num,error); //%24.16g
}

void cluster_print_summary ( int point_num, int cluster_num,
  int cluster_population[], double cluster_energy[], double cluster_variance[] ,int it_num )
...
*/
{
  ...
  log_error(point_num,cluster_num,it_num,ce_total) ;
