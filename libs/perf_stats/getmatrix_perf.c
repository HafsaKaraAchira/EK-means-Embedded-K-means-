...
./perf_stats/getmatrix_perf generator/CM13,4M_2400MO_SEP0,2/points.csv 2 13421800 6710900 10
...
double *getmatrix_buf(char *source, size_t dim, int k, size_t N, int sub, int offset, int *marks)
{
	...
	// ="0.500040325395034\t0.491522145241467\t0.469243971882188\t0.653238734557954\t0.319925902731301\t0.544123893967396\t0.526092562503245\t0.467128339965845\t0.560311065512695\t0.575082213530508\n" ;
	...
	int cpid , pid ;
		...
		while (!stop)
		{
			...
			if( cpid == 0)
			{
				...
				char msg[50];
				sprintf(msg, "perf stat -p %d   >> getmatrix_stat.log 2>&1",pid);
