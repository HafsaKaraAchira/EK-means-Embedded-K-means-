...
(scripts/prog_script_cache) && (clear && gcc -g perf_stats/form_index_perf.c -o perf_stats/form_index_perf -lm -D _GNU_SOURCE)
./perf_stats/form_index_perf generator/CM13,4M_2400MO_SEP0,2/points.csv 2 13421800 6710900 10
...
double *form_chunk(groupe *grp, /*double *X*/ char *source, int *marks, int *cluster_assignment, int nb_groupes, size_t N, size_t dim, int k, size_t taille)
{
	...
	if (cpid == 0)
	{
		...
		char msg[50];
		sprintf(msg, "perf stat -p %d   >> form_chunk_stat.log 2>&1", pid);
