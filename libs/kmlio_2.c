...
long *mark(char *source, size_t dim, size_t N, int chunk_size){
	...
	while (!feof(src)){
		...
		if (j==0){
			...
			mark_index++; 
			// printf("test mark end\n") ;
		}
	}
	printf("max line size test\n") ;
	fclose(src);
...
double * form_chunk( groupe *grp, /*double *X*/char * source, long * marks, int * cluster_assignment, int nb_groupes, size_t N, size_t dim, int k , size_t taille){
	...
	// printf("test after group shuffle\n") ;
	size=0; 
...
void kmeans_by_chunk( char * source, size_t dim, int taille, int N, int k){
	...
	if (N/taille >1){
		...
		int cluster_member_count[(int) count]; //à modifier pour une allocation dynamique 
...
void kmeans_kmeans(double * X,int dim, int taille, int N, int k){ //expérimentation de la méthode de kmeans de kmeans sans formation de chunk représentatif
	...
	int kmeans_iteration = 0,kmeans_change_count = 0;
	...
	for( m = 0; m<N/taille; m++){
		...
		cluster_centroid =kmeans_init_plusplus(Y, taille, dim, k);	        
		kmeans(dim,Y,taille, k, cluster_centroid, cluster_assignment_final_Y,&kmeans_iteration,&kmeans_change_count);
		for ( j=0; j<k;j++){
			var[m*k+j] = var_calculate (Y, dim, cluster_centroid, j, cluster_assignment_final_Y , taille)/taille; 
		}
		for ( j=0; j<k*dim;j++){
	...
	kmeans(dim,cluster_centroid_by_chunk,k*N/taille, k, cluster_centroid, cluster_assignment_final_Y,&kmeans_iteration,&kmeans_change_count);
