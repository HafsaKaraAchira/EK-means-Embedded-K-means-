...
void decide_skip_chunk(size_t N, size_t M,long * freq){
	...
	while(skp < (N/M) - chunk_ind){
		f = available_frequencies[nb_available_frequencies-1] ; //get_optimal_freq(N,M,skp,&found,&estimated_curr_chunk_delay,&rem_time) ;
		if( found == 1 ){
			skp_chk = skp ;
			(*freq) = f ;
...
void kmlio_diag(size_t k,size_t dim, size_t N, size_t taille,double D_max,double * centroid){
	...
	fprintf(fl,"%d,%d,%d,%d,%.3f,%.3f,%d,%lf,",N,taille,dim,k,D_max,beta,skp_chk,calc_delai_time(kmlio_start,kmlio_end));
